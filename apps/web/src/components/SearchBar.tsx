...
export function SearchBar({
  ...
  const variantClasses = {
    default: {
      container: 'relative',
      input: 'w-full pl-10 pr-10 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500',
      dropdown: 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-xl',
    },
    hero: {
      container: 'relative',
      input: 'w-full pl-12 pr-12 py-4 rounded-xl border-2 border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-lg placeholder-slate-400 focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 shadow-lg',
      dropdown: 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-2xl',
    },
    cosmic: {
      container: 'relative',
      input: 'w-full pl-10 pr-10 py-2.5 rounded-lg border border-[#FFD700]/20 bg-white/5 text-white placeholder-slate-400 focus:ring-2 focus:ring-[#FFD700]/50 focus:border-[#FFD700]/50 backdrop-blur-sm',
      dropdown: 'bg-[#1A0F2E] border border-[#FFD700]/20 shadow-xl',
    },
    minimal: {
      container: 'relative',
      input: 'w-full pl-9 pr-8 py-2 rounded-lg border-0 bg-slate-100 dark:bg-slate-700 text-slate-900 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500',
      dropdown: 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-xl',
    },
  };
    ...
    <div ref={containerRef} className={`${styles.container} ${className}`}>
      <form onSubmit={handleSubmit}>
        <div className="relative">
          <Search className={`absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 ${variant === 'hero' ? 'left-4' : ''}`} />
          <input
            ref={inputRef}
            type="text"
            value={query}
          ...
          {query && (
            <button
              type="button"
              onClick={() => setQuery('')}
              className={`absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-600 ${variant === 'hero' ? 'right-4' : ''}`}
            >
              <X className="w-4 h-4" />
            </button>
          )}
      ...
      {isFocused && showSuggestions && (
        <div className={`absolute z-50 w-full mt-2 rounded-xl overflow-hidden ${styles.dropdown}`}>
          ...
          {!loading && results.length > 0 && (
            <div className="py-2">
              <div className="px-4 py-2 text-xs font-semibold text-slate-500 uppercase">Results</div>
              {results.map((result) => (
                <Link
                  key={result.id}
                  href={result.href}
              ...
              <div className="py-2">
                ...
                {recentSearches.map((search) => (
                  <button
                    ...
                    className="w-full flex items-center gap-3 px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors text-left"
                  >
              ...
              <div className="py-2 border-t border-slate-100 dark:border-slate-700">
                ...
                {popularSearches.map((item) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between gap-3 px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors text-left"
                  >

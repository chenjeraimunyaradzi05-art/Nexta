...
export function JobCard({
  ...
  if (variant === 'compact') {
    return (
      <Link
        href={`/jobs/${job.slug}`}
        className={`
          block p-4 rounded-lg border border-slate-200 dark:border-slate-700
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-medium text-slate-900 dark:text-white truncate group-hover:text-blue-600">
              {job.title}
            </h3>
  ...
  if (variant === 'horizontal') {
    return (
      <Link
        href={`/jobs/${job.slug}`}
        className={`
          block p-4 sm:p-6 rounded-xl border border-slate-200 dark:border-slate-700
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-2">
            <div className="text-right">
              ...
              {job.postedDate && (
                <p className="text-xs text-slate-400">{getTimeAgo(job.postedDate)}</p>
              )}
            </div>
  ...
  return (
    <Link
      href={`/jobs/${job.slug}`}
      className={`
        group relative block p-6 rounded-xl border border-slate-100 dark:border-slate-700
    ...
    return (
      <div className={variant === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6' : 'space-y-4'}>
        {Array.from({ length: skeletonCount }).map((_, i) => (
          <div
            key={i}
            className="animate-pulse p-6 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800"
          >

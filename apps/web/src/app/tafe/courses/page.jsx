...
export default function TafeCourseManagement() {
  ...
  async function loadCourses() {
    setLoading(true);
    try {
      const res = await fetch(`/api/neon/courses?pageSize=100`);
      if (res.ok) {
  ...
  async function saveCourse() {
    try {
      ...
      showNotification({
        message: editingCourse ? 'Course updated!' : 'Course created!',
        variant: 'success',
      });
    ...
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden py-12 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-pink-50/40 to-purple-50/40 dark:from-slate-900 dark:to-slate-800">
        ...
        <div className="relative max-w-5xl mx-auto">
          <Link
            href="/tafe/dashboard"
            className="text-pink-600 hover:text-pink-700 text-sm mb-4 inline-flex items-center gap-1 font-medium"
          >
            ← Dashboard
          </Link>
          ...
          <div className="flex justify-between items-center">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.25em] text-pink-600 mb-2">
                Training Programs
              </p>
              ...
              <p className="text-lg text-slate-600 mt-2">
                Create and manage training courses for community members.
              </p>
            </div>
            <button
              onClick={openCreate}
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {courses.map((course) => (
              <div
                key={course.id}
                className="rounded-2xl border border-slate-200 bg-white p-5 shadow-lg transition-all hover:shadow-xl"
                ...
                <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-2 mb-1">
                      ...
                      {course.qualification && (
                        <span
                          ...
                          style={{ background: 'rgba(139, 92, 246, 0.1)', color: accentPurple }}
                        >
                          {course.qualification}
                        </span>
                      )}
                  ...
                  <div className="flex gap-2">
                    <Link
                      ...
                      className="px-3 py-2 border-2 rounded-lg text-sm font-semibold transition-all hover:bg-slate-50"
          ...
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 overflow-auto z-50">
            <div className="bg-white border border-slate-200 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-auto">
              ...
              <div className="p-5 space-y-4">
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-[0.15em] text-pink-600 mb-2">
                    Title *
                  </label>
                  <input
                    value={form.title}
                ...
                <div className="flex justify-end gap-3 pt-4">
                  <button
                    onClick={() => setShowCreateModal(false)}
                    className="px-5 py-2.5 border-2 border-slate-200 rounded-lg font-semibold text-slate-600 hover:bg-slate-50 transition"
                  >
                    Cancel
                  </button>
                  <button
                    onClick={saveCourse}
                    className="px-6 py-2.5 rounded-lg text-white font-semibold transition-all hover:-translate-y-0.5"
                    style={{ background: 'linear-gradient(135deg, #E91E8C 0%, #8B5CF6 100%)' }}
                  >

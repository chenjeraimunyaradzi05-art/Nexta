...
export default function PrivacyPage() {
  ...
  if (loading) {
    return (
      <div className="nexta-page max-w-4xl mx-auto px-4 py-16">
        <div className="flex items-center gap-3">
          ...
          <span className="nexta-muted">Loading privacy & data settings…</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="nexta-page max-w-4xl mx-auto px-4 py-8">
      <nav className="mb-6 text-sm" aria-label="Breadcrumb">
        <ol className="flex items-center gap-2 nexta-muted">
          <li><Link href="/" className="nexta-link">Home</Link></li>
          <li><span className="text-slate-400 dark:text-slate-600">/</span></li>
          <li className="text-slate-900 dark:text-white">Privacy & Data</li>
        </ol>
      </nav>

      <h1 className="nexta-h1 mb-2">Privacy & Data</h1>
      <p className="nexta-text mb-8">
      ...
      <section className="nexta-card p-6 mb-6">
        <h2 className="nexta-h2 text-xl mb-1">Consent Preferences</h2>
        <p className="nexta-muted text-sm mb-4">Last updated: {consentUpdatedLabel}</p>
          ...
          {[
            ...
            <label key={key} className="flex items-start justify-between gap-4 border border-slate-200 dark:border-slate-700 rounded-lg p-4">
              <div>
                ...
                <div className="text-sm nexta-muted">{desc}</div>
              </div>
              <input
                type="checkbox"
      ...
      <section className="nexta-card p-6 mb-6">
        <h2 className="nexta-h2 text-xl mb-2">Community Data Benefit Agreement</h2>
        <p className="nexta-text mb-4">{cdba?.description || 'Anonymized, aggregated insights can help benefit community outcomes.'}</p>

        {cdba?.benefits?.length ? (
          <div className="text-sm nexta-text space-y-1">
            {cdba.benefits.map((b) => (
              <div key={b}>• {b}</div>
            ))}
          </div>
      ...
      <section className="nexta-card p-6 mb-6">
        <h2 className="nexta-h2 text-xl mb-2">Export Your Data</h2>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          <button
            ...
            className="nexta-btn-primary px-4 py-2 rounded-lg"
          >
            Request JSON Export
          </button>
          <button
            ...
            className="nexta-btn-secondary px-4 py-2 rounded-lg"
          >
            Request CSV Export
          </button>
        </div>
        ...
        {exportId && (
          <div className="border border-slate-200 dark:border-slate-700 rounded-lg p-4">
            <div className="text-sm nexta-text mb-2">Export ID: <span className="text-slate-900 dark:text-slate-200">{exportId}</span></div>
            <div className="text-sm nexta-muted mb-4">Status: {exportStatus?.status || 'unknown'}</div>
      ...
      <section className="nexta-card border-red-200 dark:border-red-900/40 p-6">
        <h2 className="nexta-h2 text-xl mb-2">Account Deletion</h2>
        <p className="nexta-muted text-sm mb-4">

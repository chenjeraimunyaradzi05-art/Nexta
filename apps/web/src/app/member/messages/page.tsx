    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-slate-900 dark:text-white mb-2">Sign in to view messages</h2>
          <p className="text-slate-500 dark:text-slate-400 mb-4">You need to be logged in to access your messages.</p>
  ...
  return (
    <div className="h-[calc(100vh-4rem)] bg-slate-50 dark:bg-slate-950 flex flex-col">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className={`w-full md:w-80 lg:w-96 border-r border-slate-200 dark:border-slate-800 flex flex-col bg-white dark:bg-slate-900 ${showMobileChat ? 'hidden md:flex' : 'flex'}`}>
          {/* Header */}
          <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex-shrink-0">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                {totalUnread > 0 && (
                  <p className="text-xs text-slate-500 dark:text-slate-400">{totalUnread} unread</p>
                )}
              </div>
              <button
              ...
              <div className="mb-3 p-3 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg border border-emerald-200 dark:border-emerald-700/40">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:border-emerald-500 outline-none"
                    autoFocus
                  />
                  <button
                    onClick={async () => {
                  ...
                  <button
                    ...
                    className="px-2 py-1.5 text-slate-500 dark:text-slate-400 text-sm hover:text-slate-700 dark:hover:text-white"
                  >
                    ×
                  </button>
                </div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:border-emerald-500 outline-none"
              />
            </div>
            {/* Filters */}
                      ...
                      <div className="flex gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            {conv.isMuted && (
                              <BellOff className="w-3 h-3 text-slate-400 dark:text-slate-500" />
                            )}
                          </div>
                        </div>
                      </div>
        ...
        {/* Chat Area */}
        <div className={`flex-1 flex flex-col bg-slate-50 dark:bg-slate-950 ${!showMobileChat ? 'hidden md:flex' : 'flex'}`}>
          {selectedConversation ? (
            <>
              {/* Chat Header */}
              <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center gap-3 flex-shrink-0 bg-white dark:bg-slate-900">
                {/* Mobile back button */}
                <button
                        ...
                        <React.Fragment key={msg.id}>
                          ...
                          <div className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[75%]`}>
                              ...
                              {!isMe && (
                                <p className="text-xs text-slate-500 dark:text-slate-500 mb-1 ml-1">{msg.senderName}</p>
                              )}
                              <div className={`px-4 py-2.5 rounded-2xl ${
                                isMe
                                  ? 'bg-emerald-600 text-white rounded-br-md'
                                  : 'bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-200 rounded-bl-md border border-slate-200 dark:border-slate-700'
                              }`}>
                                <p className="text-sm leading-relaxed">
                                  {msg.isDeleted ? <em className="text-slate-400 dark:text-slate-500">Message deleted</em> : msg.content}
                                </p>
                              </div>
                              <p className={`text-xs text-slate-400 dark:text-slate-500 mt-1 ${isMe ? 'text-right' : 'text-left'}`}>
              ...
              <div className="p-4 border-t border-slate-200 dark:border-slate-800 flex-shrink-0 bg-white dark:bg-slate-900">
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="flex-1 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2.5 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:border-emerald-500 outline-none text-sm"
                  />
                  <button
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                <MessageSquare className="w-16 h-16 text-slate-300 dark:text-slate-600 mx-auto" />
                <h3 className="text-xl font-medium text-slate-900 dark:text-white mt-4">Select a conversation</h3>

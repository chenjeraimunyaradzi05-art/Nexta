...
export default function JobDetailsSEO() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        const res = await fetch(`/api/neon/jobs/${id}`);
        const json = await res.json();
        if (res.ok) setJob(json.job);
        if (token) {
          try {
            const f = await fetch(`${API_BASE}/uploads/me`, {
  ...
  async function apply() {
    setMessage(null);
    if (!token) return setMessage('Please login to apply');
    try {
      const res = await fetch(`${API_BASE}/jobs/${id}/apply`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ resumeId: selectedResume, coverLetter: cover }),
      });
      const json = await res.json();
  ...
  if (!job)
    return (
      <div className="max-w-4xl mx-auto py-12">
        <a href="/jobs" className="text-blue-300 hover:text-blue-200 text-sm mb-4 inline-block">
          ← All jobs
        </a>
        <div className="text-slate-300 bg-slate-900/40 border border-slate-800 p-6 rounded">
        ...
        <section className="mt-6">
          ...
          {!user ? (
            <div className="text-sm text-slate-200">
              Sign in with a Member account to apply for this role.
            </div>
          ) : user.userType !== 'MEMBER' ? (
            <div className="text-sm text-slate-200">
              Only Member accounts can apply. Create a Member account to apply.
            </div>
          ) : (
            <div className="space-y-3">
              ...
              <label className="block">
                <span className="text-sm text-slate-200">Cover letter (optional)</span>
                <textarea
                  value={cover}
                  onChange={(e) => setCover(e.target.value)}
                  placeholder="Introduce yourself and explain why you're interested in this role…"
                  className="w-full mt-1 border border-slate-700 bg-slate-950/40 px-3 py-2 rounded h-28 text-slate-100 placeholder:text-slate-500"
                />
              </label>

              <div className="flex justify-between items-center">
                <div className="text-sm text-slate-300">
                  You may upload a resume via your Member Dashboard / Resume
                </div>
                <button
                  onClick={apply}

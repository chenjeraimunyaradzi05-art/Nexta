...
export default function JobDetails() {
  ...
  async function apply() {
    setMessage(null);
    if (!token) return setMessage({ type: 'error', text: 'Please login to apply' });
    try {
      setApplying(true);
  ...
  if (!job)
    return (
      <div className="max-w-4xl mx-auto py-12 px-4">
        <Link href="/jobs" className="text-blue-300 hover:text-blue-200 text-sm mb-4 inline-block">
          ← All jobs
        </Link>
        ...
        <div className="bg-slate-900/40 border border-slate-800 p-4 rounded-lg mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <div className="text-sm text-slate-400">
              Compare your skills against this role and find training to close the gap.
            </div>
          </div>
          <Link
            ...
            <div>
              ...
              {files.length === 0 && (
                <p className="text-xs text-slate-400 mt-1">
                  No resumes found.{' '}
                  <Link href="/member/dashboard" className="text-blue-400 underline">
                    Upload one
                  </Link>{' '}
                  first.
                </p>

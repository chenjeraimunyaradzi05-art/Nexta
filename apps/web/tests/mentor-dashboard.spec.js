...
test('Mentor dashboard fetches AiConcierge suggestions on load', async ({ page, request }) => {
    ...
    await page.route(`${apiBase}/ai/concierge`, async (route) => {
        ...
        await route.fulfill({
            ...
            body: JSON.stringify({ source: 'prototype', suggestions: ['Deterministic mentor suggestion'] }),
        });
    });
    ...
    await page.waitForSelector('text=AiConcierge suggestions', { timeout: 15000 });
    await page.waitForSelector('text=AI-powered', { timeout: 5000 });
    const suggestions = await page.locator('text=AiConcierge suggestions').first().innerText();
    expect(suggestions).toContain('AiConcierge suggestions');
});
